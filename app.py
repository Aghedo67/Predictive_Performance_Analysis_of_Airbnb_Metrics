...
def load_data():
    ...
    raw_df = pd.read_csv('dublin_airbnb_df.csv')
...
@st.cache_resource
def load_model():
    return joblib.load('Igb_model_log.pkl')
...
def suggest_price(model, dublin_aggregated_df):
    with st.sidebar:
        ...
        neighbourhood_cleansed = st.selectbox("Neighbourhood:", dublin_aggregated_df['neighbourhood_cleansed'].unique())
        avg_rating = st.number_input("Rating:", 0.0, 5.0, 4.5)
        number_of_reviews = st.number_input("Reviews:", 0, 1000, 10)

        if st.button("Calculate Price"):
            # Validate and calculate
            if neighbourhood_cleansed in dublin_aggregated_df['neighbourhood_cleansed'].values and \
